...
def pregunta_3(lat,long):
     query = """
        ...
        PREFIX sf: <https://saref.etsi.org/core/>
        ...
        SELECT distinct ?container ?addressname ?distance { 
            SERVICE <http://localhost:7200/repositories/app> {
                ...
                ?address pr:hasFullAdress ?addressname .
                BIND( STR("POINT($long$ $lat$)^^geo:wktLiteral") AS ?my_position )
                BIND( geof:distance(?container_position, ?my_position, opgs:metre) AS ?distance ) .
                Filter(?distance < 300)
            }
        }
        ORDER BY ?distance
...
if x == '2':
    ...
    print('Vicalvaro:19')
    print('Villalba:20')
    y = input('Escoja uno de los anteriores números:')
    print(pregunta_2(y))
if x == '3':
